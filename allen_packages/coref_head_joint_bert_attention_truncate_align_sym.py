...
from .custom_berts import BertModelwAttention


logger = logging.getLogger(__name__)


@Model.register("coref_head_joint_bert_attention_truncate_align_sym")
class CoreferenceResolver(Model):
    ...
    def __init__(
        ...
        if bert_feedforward is not None:
            ...
            self._align_attention = BilinearMatrixAttention(self._bert_feedforward.get_output_dim(), self._bert_feedforward.get_output_dim())
        else:
            ...
            self._align_attention = BilinearMatrixAttention(self._context_layer.get_output_dim(), self._context_layer.get_output_dim())
    ...
    def forward(
        ...
        # Shape: (batch_size, document_length, document_length + 1)
        align_matrix = self._align_attention(text_embeddings, text_embeddings)
        ...
        weighted_all_attention = torch.matmul(align_matrix, weighted_all_attention)
        weighted_all_attention = torch.matmul(weighted_all_attention, align_matrix.transpose(1, 2))
        ...
        head_link_matrix = (head_link_matrix + head_link_matrix.transpose(1, 2)) / 2
        ...
        if span_labels is not None:
            ...
            gold_bidirectional_labels = self._compute_bidirectional_gold_labels(span_labels)
            ...
            correct_antecedent_log_probs = coreference_log_probs + gold_bidirectional_labels.log()
