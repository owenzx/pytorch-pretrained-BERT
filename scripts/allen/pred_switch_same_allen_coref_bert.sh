...
allennlp predict --use-dataset-reader --predictor coreference-resolution --cuda-device 0 --silent --overrides='{"dataset_reader":{"cached_instance_path":"./switch.ist.3"}}' --output-file ./outputs/debug.out --include-package allen_packages ./outputs/allen_test_bert_tune_large/model.tar.gz ./datasets/coref/allen/dev.english.v4_gold_conll
