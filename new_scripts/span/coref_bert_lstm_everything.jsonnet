...
local inp_feature_dim = 20;

local bert_dim = 768;  # uniquely determined by bert_model
local span_embedding_dim = 3 * (bert_dim + 2 * inp_feature_dim) + feature_size;
...

{
  ...
  "model": {
    ...
    "speaker_field_embedder":{
      "token_embedders":{
        "speakers":{
          "type": "embedding",
          "embedding_dim": inp_feature_dim,
          "vocab_namespace": "speakers",
        },
      },
    },
    "genre_field_embedder":{
      "token_embedders":{
        "genres":{
          "type": "embedding",
          "embedding_dim": inp_feature_dim,
          "vocab_namespace": "genres",
        },
      },
    },
    "context_layer": {
        "type": "pass_through",
        "input_dim": bert_dim + 2 * inp_feature_dim,
    },
    "mention_feedforward": {
