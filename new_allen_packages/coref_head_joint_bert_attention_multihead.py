...
from .mh_attention import MHAttention as MultiheadAttention
...
@Model.register("coref_head_joint_bert_attention_mh")
class CoreferenceResolver(Model):
    ...
    def __init__(
        ...
        if bert_feedforward is not None:
            ...
            self._final_link_attention = MultiheadAttention(self._bert_feedforward.get_output_dim(), num_heads, 0.0)
        else:
            self._bert_feedforward = None
            if context_layer is not None:
                self._final_link_attention = MultiheadAttention(self._context_layer.get_output_dim(), num_heads, 0.0)
            else:
                self._final_link_attention = MultiheadAttention(768, num_heads, 0.0)
        ...
        self._conll_coref_scores = ConllHeadPruneCorefScores()
    ...
    def forward(
        ...
        sn_embeddings = top_head_embeddings.transpose(0,1)
        head_link_matrix = self._final_link_attention(sn_embeddings, sn_embeddings, sn_embeddings)[1]
       ...
       # concat with dummy index
        ...
        flat_predicted_antecedents = util.flatten_and_batch_shift_indices(predicted_antecedents, top_head_indices_wdummy.shape[1])

        original_antecedent_indices = util.batched_index_select(top_head_indices_wdummy.unsqueeze(-1), predicted_antecedents, flat_predicted_antecedents).squeeze(-1)

        # print(predicted_antecedents[0])
        # print(original_antecedent_indices[0])
        # print(top_head_indices[0])
        # exit()
