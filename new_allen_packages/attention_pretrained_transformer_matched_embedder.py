...
@TokenEmbedder.register("attention_pretrained_transformer")
class AttentionPretrainedTransformerEmbedder(TokenEmbedder):
    """
    ...
    def __init__(self, model_name: str, max_length: int = None) -> None:
        ...
        transformer_config = AutoConfig.from_pretrained(model_name, output_attentions=True)
        self.transformer_model = AutoModel.from_pretrained(model_name, config=transformer_config)
        print(self.transformer_model.config)
    ...
    def forward(
        ...
        # TODO Try to get the attention out here
    ...
    def _unfold_long_attentions(self,
        ...
        # Shape of attention is [b*seg, #num_att, max_len1, max_len2]
        device = attentions.device
        num_att = attentions.size(1)
        num_segments = int(attentions.size(0) / batch_size)

        max_len = attentions.size(-1)

        attentions = attentions.contiguous()
        attentions = attentions.unsqueeze(1).repeat(1, num_segments, 1, 1, 1).view(batch_size, num_segments, num_segments,num_att, max_len, max_len)




        self_seg_mask = torch.eye(num_segments, device=device)
        # Shape [seg_num, seg_num]
        self_seg_mask = self_seg_mask.view(1, num_segments, num_segments, 1, 1, 1)
        # Shape [1, seg_num, seg_num, 1, 1, 1]


        attentions = attentions * self_seg_mask

        attentions = attentions.transpose(2, 4)
        # Shape [batch_size, num_seg, max_len1, num_att, num_seg, max_len2]

        attentions = attentions.reshape(batch_size*num_segments, max_len, num_att* num_segments * max_len)

        part_unfold_attentions = self._unfold_long_sequences(attentions, mask, batch_size, num_segment_concat_wordpieces)
        # Shape [b, total_len1, num_att * num_seg * max_len2]

        total_len = part_unfold_attentions.shape[1]

        part_unfold_attentions = part_unfold_attentions.view(batch_size, -1, num_att, num_segments * max_len).transpose(1, 3)
        # Shape [b, num_seg * max_len2, num_att, total_len1]

        tmp_attentions = part_unfold_attentions.reshape(batch_size * num_segments, max_len, -1)
        # Shape [b*num_seg, max_len2, num_att * total_len1]

        unfold_attentions = self._unfold_long_sequences(tmp_attentions, mask, batch_size, num_segment_concat_wordpieces)

        unfold_attentions = unfold_attentions.view(batch_size, total_len, num_att, total_len).transpose(1,2).transpose(2, 3)

        return unfold_attentions