...
@DatasetReader.register("coref_head_joint_bert_af")
class ConllCorefReaderHeadBert(DatasetReader):
    ...
    def __init__(
        ...
        try:
            assert(self._wordpiece_modeling_tokenizer==None)
        except:
            print("WORDPIECE_MODELING_NOT_IMPLEMENTED")
            exit()
            ...
            if self._truncation or self._max_sentences is None:
                ...
                if len(sentences) > self._max_sentences:
                    ...
                    for idx in start_idxs:
                        yield self.text_to_instance([s for s in sentences], canonical_clusters, idx)
                else:
                    yield self.text_to_instance([s for s in sentences], canonical_clusters)
    ...
    def text_to_instance(
        ...
        for s_idx, sentence in enumerate(sentences):
            normalized_words = [self._normalize_word(word) for word in sentence]
            if self._wordpiece_modeling_tokenizer is not None:
                sentence_tokens, sentence_offsets = self._wordpiece_modeling_tokenizer.intra_word_tokenize(
                    normalized_words
                )
                ...
                flat_speakers.extend([speakers[s_idx][0] for _ in sentence])
                flat_genres.extend([genres[s_idx] for _ in sentence])
