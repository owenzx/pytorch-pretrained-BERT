...
@Model.register("PG_seq2seq_wparse")
class PGSeq2Seq_wparse(Model):
    def __init__(self,
                 vocab: Vocabulary,
        ...
        super(PGSeq2Seq_wparse, self).__init__(vocab)
        ...
        self._encoder2_output_dim = self._encoder2.get_output_dim()
        #assume concat
        self._decoder_output_dim = self._encoder_output_dim + self._encoder2_output_dim
    ...
    def forward(self,  # type: ignore
                ...
                source_token2: Dict[str, torch.LongTensor],
        ...
        print(source_tokens)
        print(source_token2)
        state1 = self._encode(source_tokens, source_span)
        state2 = self._encode2(source_token2)

        print("STATES")
        print(state1['encoder_outputs'].shape)
        print(state2['encoder_outputs'].shape)
        #state = {"source_mask": state1['source_mask'], "encoder_outputs":torch.cat((state1['encoder_outputs'], state2['encoder_outputs']), -1)}

        if target_tokens:
            state = self._init_decoder_state(state1, state2)
    ...
    def _encode2(self, source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        # shape: (batch_size, max_input_sequence_length, encoder_input_dim)
        embedded_input = self._source_embedder(source_tokens)
        # shape: (batch_size, max_input_sequence_length)
        source_mask = util.get_text_field_mask(source_tokens)
        # shape: (batch_size, max_input_sequence_length, encoder_output_dim)
        encoder_outputs = self._encoder2(embedded_input, source_mask)
        return {
                "source_mask": source_mask,
                "encoder_outputs": encoder_outputs,
        }
    ...
    def _init_decoder_state(self, state1: Dict[str, torch.Tensor], state2: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        state = {}
        batch_size = state1["source_mask"].size(0)
        # shape: (batch_size, encoder_output_dim)
        final_encoder_output1 = util.get_final_encoder_states(
                state1["encoder_outputs"],
                state1["source_mask"],
                self._encoder.is_bidirectional())
        final_encoder_output2 = util.get_final_encoder_states(
            state2["encoder_outputs"],
            state2["source_mask"],
            self._encoder2.is_bidirectional())

        final_encoder_output = torch.cat((final_encoder_output1, final_encoder_output2), -1)

        # Initialize the decoder hidden state with the final output of the encoder.
        # shape: (batch_size, decoder_output_dim)
        state["source_mask"] = state1["source_mask"]
        state["encoder_outputs"] = state1["encoder_outputs"]
        state["decoder_hidden"] = final_encoder_output
        # shape: (batch_size, decoder_output_dim)
        state["decoder_context"] = state1["encoder_outputs"].new_zeros(batch_size, self._decoder_output_dim)
        return state
