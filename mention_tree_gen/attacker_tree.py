...
class AttackerTree(object):
    ...
    def sample_new_mention(self, instance):
        #self.training = False
    ...
    def beam_search_new_mention(self, instance):
    ...
    def decode_best_mention(self):
        pass




class EditAttackerTree(object):
    def __init__(self, args, vocab: Vocabulary):
        # init client
        CUSTOM_PROPS = {'tokenize.whitespace':True}
        self.client = CoreNLPClient(annotators=['tokenize', 'ssplit', 'coref'], timeout=6000000, memory='16G', be_quiet=True, properties=CUSTOM_PROPS)
        ...
        #self.valid_cfgs = ['STOP', 'NP->N', 'NP->AP,N', 'N->N,PP', 'NP->AP,N,PP', 'AP->AP,AP', 'AP->A', 'AP->Adv,A', 'PP->PP,PP', 'PP->P,NP']
        self.valid_cfgs = ['@start@', '@end@', 'STOP', 'NP->N', 'NP->AP,N', 'N->N,PP', 'NP->AP,N,PP', 'AP->AP,AP', 'AP->A', 'AP->Adv,A', 'PP->PP,PP', 'PP->P,NP']
        self.valid_cfgs = ['@start@', '@end@', 'STOP', 'NP:add ADJP', 'NP:add PP', 'NP:reduce', 'ADJP:delete', 'PP:delete', 'ADJP:expand A', 'ADJP:expand Adv,A', 'PP:expand P,N', 'NP:confirm', 'ADJP:confirm', 'PP:confirm']
        ...
        #additional encoder for linearized parsing
        encoder2 = PytorchSeq2SeqWrapper(torch.nn.LSTM(input_size=args.embedding_dim,
                                         hidden_size=args.encoder_dim,
                                         num_layers=1,
                                         batch_first=True,
                                         bidirectional=True))
        ...
        self.controller = seq2seq_parse(vocab=vocab,
                                  source_embedder=embedder,
                                  encoder=encoder,
                                  encoder2=encoder2,
                                  max_decoding_steps=args.max_decoding_steps,
    ...
    def sample_new_mention(self, instance):
        ...
        print(instance[0]['metadata'][0]['original_parse'])
        controller_output = self.controller.forward(instance[0]['text'], instance[0]['selected_mentions'], instance[0]['parse'], sample=True, get_prediction=True)
        controller_output_2 = self.controller.decode(controller_output)
        #self.controller.train()
        return controller_output_2
    ...
    def beam_search_new_mention(self, instance):
        self.lexicon_filler.set_inital_tree(instance[0]['metadata'][0]['original_parse'])
        controller_output = self.controller.forward(instance[0]['text'], instance[0]['selected_mentions'], instance[0]['parse'], get_prediction=True)
        controller_output_2 = self.controller.decode(controller_output)
        return controller_output_2
    ...
    def get_new_mention_text(self, decoder_outdict, metadata):
        ...
        cfgs = decoder_outdict['predicted_tokens'][0]
        #new_mention_text = self.lexicon_filler.fill_lexicon(cfgs, whole_sentence, np_head)
        new_mention_text = self.lexicon_filler.fill_lexicon_edit_tree(whole_sentence)
        return new_mention_text
